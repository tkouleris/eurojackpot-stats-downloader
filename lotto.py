...
URL = "https://www.allwyn.gr/el/lotto/draws-results"
TARGET_YEAR = str(datetime.now().year)
OUTPUT_DIR = Path(__file__).parent / "downloads"
LOG_FILE = Path(__file__).parent / "log_lotto.txt"

load_dotenv()

DEST_PATH = os.getenv("LOTTO_DEST_PATH")
CACHE_COMMAND = os.getenv("LOTTO_CACHE_COMMAND")
MAIN_DEST_PATH = os.getenv("MAIN_DEST_PATH")
DEST_OWNER = os.getenv("OWNER")
DEST_GROUP = os.getenv("GROUP")


def download_lotto_draws(
    url: str = URL,
    ...
    """
    Επισκέπτεται τη σελίδα https://www.allwyn.gr/el/lotto/draws-results
    και κατεβάζει το αρχείο κληρώσεων Λοττο για το επιλεγμένο έτος (π.χ. 2026).
    """
    output_dir.mkdir(parents=True, exist_ok=True)
    print(f"[*] Εκκίνηση διαδικασίας λήψης κληρώσεων Λοττο για το έτος {year}...")
    log_to_file(f"[*] Εκκίνηση διαδικασίας λήψης κληρώσεων Λοττο για το έτος {year}...", LOG_FILE)
        ...
        with sync_playwright() as p:
            ...
            if browser:
                ...
                try:
                    ...
                    # Εντοπισμός του dropdown επιλογής έτους στο πεδίο «Αρχείο Αποτελεσμάτων» (.download select, select.archive)
                    download_select = page.locator("select.archive, .download select, div.download select").first
                    if not download_select or download_select.count() == 0 or not download_select.is_visible(timeout=2000):
                        # Εναλλακτικός εντοπισμός επιλογέα έτους αρχείου
                        candidates = page.locator("select[aria-label='Έτος'], select.archive")
                        if candidates.count() > 1:
                            download_select = candidates.last
                    ...
                    if download_select and download_select.count() > 0:
                        ...
                        try:
                            ...
                            suggested_filename = download.suggested_filename or f"lotto_{year}.xlsx"
    ...
    if not downloaded_file_path or not downloaded_file_path.exists() or downloaded_file_path.stat().st_size == 0:
        direct_url = f"https://media.opap.gr/Excel_xlsx/5103/lotto_{year}.xlsx"
        ...
        try:
            ...
            save_path = output_dir / f"lotto_{year}.xlsx"
...
def copy_to_env_path(file_path: Path) -> Path | None:
    ...
    if not DEST_PATH:
        print("[!] Δεν έχει οριστεί το LOTTO_DEST_PATH στο .env")
        log_to_file("[!] Δεν έχει οριστεί το LOTTO_DEST_PATH στο .env")
        return None
...
def main():
    parser = argparse.ArgumentParser(description="Λήψη αρχείου αποτελεσμάτων Λόττο από το allwyn.gr")
